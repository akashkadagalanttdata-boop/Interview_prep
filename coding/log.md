...
|---|---|---|
| `heapq` (incl. max-heap trick, `nlargest`) | no | — |
| `bisect` (`bisect_left` vs `right`, `key=`) | no | — |
| `collections` (`deque`, `defaultdict`, `Counter`) | no | — |
| `itertools` (`accumulate`, `pairwise`, `product`) | no | — |
| `functools.lru_cache` / `cache` for memoization | no | — |
| Sharp edges: mutable default args, `//` on negatives, sort stability | no | — |
